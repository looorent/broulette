...
import {
  TripAdvisorEmptyResponseError,
  TripAdvisorExceedsNumberOfCallsError,
  TripAdvisorHttpError,
  TripAdvisorLocationNotFoundError,
  TripAdvisorServerError
} from "./error";
...
export class TripAdvisorRestaurantRepository {
  ...
  ): Promise<TripAdvisorLocationSearchResult[]> {
    return await createCircuitBreaker(() =>
      findLocationsNearbyWithoutRetry(
        this.numberOfCalls++,
        this.maximumNumberOfCalls,
        this.apiKey,
        searchQuery,
        latitude,
        longitude,
  ...
  async findLocationDetails(
    locationId: number,
    language: string | undefined,
    currency: string | undefined
  ): Promise<TripAdvisorLocation | undefined> {
    return await createCircuitBreaker(() =>
      findLocationDetailsWithoutRetry(
        this.numberOfCalls++,
        this.maximumNumberOfCalls,
        this.apiKey,
        locationId,
...
async function findLocationsNearbyWithoutRetry(
  ...
  if (numberOfCalls > maximumNumberOfCalls) {
    throw new TripAdvisorExceedsNumberOfCallsError(
    ...
    console.info(
      `[TripAdvisor] Search closest locations nearby this query '${query}'...`
    );

    const start = Date.now();
    const response = await fetch(
      `${LOCATION_URL}/search?key=${encodeURIComponent(apiKey)}&${query}`,
      {
        method: "GET",
        headers: HEADERS,
    ...
    if (response.ok) {
      console.info(
        `[TripAdvisor] Search closest locations nearby this query '${query}': done in ${durationInMs} ms.`
      );
      const body = (await response.json()) as any;
      if (body) {
        return body?.data
          ?.map(parseTripAdvisorLocationSearchResult)
          .filter(Boolean);
      } else {
...
async function findLocationDetailsWithoutRetry(
  numberOfCalls: number,
  maximumNumberOfCalls: number,
  apiKey: string,
  locationId: number,
  ...
  if (numberOfCalls > maximumNumberOfCalls) {
    ...
    console.info(
      `[TripAdvisor] Search specific location by id '${locationId}'...`
    );
    ...
    if (response.ok) {
      console.info(
        `[TripAdvisor] Search specific location by id '${locationId}': done in ${durationInMs} ms.`
      );
      const body = (await response.json()) as any;
      if (body) {
      ...
      throw new TripAdvisorLocationNotFoundError(
        locationId,
        query,
        response.status,
        await response.text(),
...
function parseTripAdvisorLocationSearchResult(
  json: any
...
function parseTripAdvisorLocation(json: any): TripAdvisorLocation | undefined {
  if (json) {
    return new TripAdvisorLocation(
      ...
      Object.fromEntries(
        Object.entries(json.review_rating_count ?? {}).map(([k, v]) => [
          k,
          parseInt(v as string) || 0
        ])
      ),
