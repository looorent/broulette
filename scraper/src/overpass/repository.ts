...
const OVERPASS_API_INSTANCE_URL = process.env["OVERPASS_API_INSTANCE_URL"] || "https://overpass-api.de/api/interpreter";

function createQueryToListAllRestaurantsInCountry(countryCodeInIso3166: string, timeoutInSeconds: number): string {
    return `
...
function parseRestaurantFromResponse(body: any): OverpassRestaurant | undefined {
    if (body) {
...
function parseResponse(body: any, durationInMs: number): OverpassResponse | undefined {
    if (body) {
...
export async function fetchAllRestaurantsInCountry(countryCodeInIso3166: string, timeoutInSeconds: number): Promise<OverpassResponse | undefined> {
    ...
    if (response.ok) {
        ...
        if (body) {
            ...
            throw new OsmEmptyResponseError(query, response.status, await response.text(), durationInMs);
        }
...
export async function fetchAllRestaurantsWithRetriesInCountry(countryCodeInIso3166: string, timeoutInSeconds: number): Promise<OverpassResponse | undefined> {
    return await createCircuitBreaker(() => fetchAllRestaurantsInCountry(countryCodeInIso3166, timeoutInSeconds));
}
