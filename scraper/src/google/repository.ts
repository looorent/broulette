...
export class GoogleRestaurantRepository {
  ...
  async findRestaurantIdentifierNearby(
    latitude: number,
    longitude: number,
    radiusInMeters: number
  ): Promise<string | undefined> {
    const places = await this.findPlacesNearby(
      latitude,
      longitude,
      radiusInMeters,
      1,
  ...
  async findRestaurantsNearby(
    ...
    return (
      (
        await this.findPlacesNearby(
          latitude,
          longitude,
          radiusInMeters,
          maximumNumberOfResultsToQuery,
  ...
  private async findPlacesNearby(
    latitude: number,
    longitude: number,
    radiusInMeters: number,
    maximumNumberOfResultsToQuery: number,
    ...
    if (this.numberOfCalls > this.maximumNumberOfCalls) {
      throw new GoogleExceedsNumberOfCallsError(
  ...
  async findRestaurantIdentifierByText(
    searchableText: string,
    latitude: number,
    longitude: number,
    radiusInMeters: number
  ): Promise<string | undefined> {
    const places = await this.findPlacesByText(
      searchableText,
      latitude,
      longitude,
      radiusInMeters,
  ...
  private async findPlacesByText(
    searchableText: string,
    latitude: number,
    longitude: number,
    radiusInMeters: number,
