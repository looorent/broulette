...
export async function validateRestaurant(restaurant: Restaurant, instant: Date): Promise<RestaurantValidation> {
  ...
  } else if (isOpenAtTarget(restaurant.openingHours, restaurant.latitude?.toNumber(), restaurant.longitude?.toNumber(), restaurant.countryCode, restaurant.state, instant) === false) {
